...
for(const permitted of [true,false,null])test('Verzending hercontroleert toestemming: '+permitted,async t=>{
 ...
 t.mock.method(globalThis,'fetch',async(url)=>{assert.equal(url,'https://api.resend.com/emails');sent++;return {ok:true,json:async()=>({id:'test-mail'})}});
 const db=async(a,p)=>{
  ...
  if(a==='job_context')return {job,customer:{id:'c',name:'Test',email:'qa@customer.be',status:'active'}};
