...
| Planning | Op verzoek van de eigenaar: 24/7, alle Belgische postcodes. Afspraken over middernacht ondersteund. | Voor de eerste boeking ontbreken nog klant, dienst, adres en gewenst tijdstip. Bestaande limieten blijven gelden: 12 uur vooraf, maximaal 4 afspraken per dag, 90 dagen vooruit. |
...
## Planning uitgebreid op verzoek van de eigenaar

`all_postcodes` en `open_24_7` zijn ingeschakeld. Instellingen kunnen via twee schakelaars in het beheer worden gewijzigd. De database blijft duur, buffer, blokkades, conflicten, boekingshorizon en daglimiet controleren. De SQL-regressietest voor middernacht, landelijke postcodes, blokkades en terugschakelen naar beperkte opening slaagt binnen BEGIN/ROLLBACK; er zijn geen testboekingen opgeslagen. De bestaande Supabase-advisormeldingen over de servergeheim-beveiligde RPC-gateway zijn ongewijzigd; zie [Supabase-controle](https://supabase.com/docs/guides/database/database-linter?lint=0028_anon_security_definer_function_executable).
