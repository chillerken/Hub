...
- 6 Python voice tests passed: duplicate tool event, failed booking creates callback, webhook signature rejection, no false live-ready/recording claim.
...
- Real production OpenAI model access probe returned HTTP 200. The email domain-list probe returned HTTP 401; this alone does not prove sending is broken because Resend send-only keys are denied domain management. Receipt in a real mailbox remains unverified.
