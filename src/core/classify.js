...
function localFallback(text,reason){return {...classifyRules(text),fallback_from:reason||'local_rules'};}
module.exports=function classifier(config){return async function classify(text){
 if(config.aiMode==='rules')return localFallback(text,'rules_mode');
 if(!config.openaiKey)return localFallback(text,'not_configured');
 ...
 try{
  ...
  if(!r.ok)return localFallback(text,providerError(r.status,d));
  if(d.status==='incomplete')return localFallback(text,'incomplete_output');
  const output=(d.output||[]).flatMap(x=>x.content||[]).filter(x=>x.type==='output_text').map(x=>x.text).join('');
  try{return {...schema.parse(JSON.parse(output)),available:true,mode:'ai'};}catch{return localFallback(text,'invalid_output');}
 }catch(e){return localFallback(text,transportError(e));}
};};
