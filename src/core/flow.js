...
function integrations(config,checks){
 ...
 {name:'E-mail',status:(gmail||outlook)?'LIVE':config.resend.apiKey&&config.resend.from?'INGESTELD':'GEBOUWD MAAR NOG NIET GEKOPPELD',detail:gmail&&outlook?'Gmail en Outlook zijn als actieve LuxWash-kanalen geverifieerd.':gmail?'Gmail-koppeling is actief.':outlook?'Outlook-koppeling is actief.':config.resend.apiKey&&config.resend.from?'Server-side verzendconfiguratie is aanwezig, maar dat bewijst nog geen werkende connector.':'Geen actieve e-mailkoppeling geregistreerd.'},
 {name:'Inkomende e-mail',status:(gmail||outlook)?'LIVE':process.env.RESEND_WEBHOOK_SECRET?'INGESTELD':'GEBOUWD MAAR NOG NIET GEKOPPELD',detail:outlook&&gmail?'Reacties kunnen via de gekoppelde Outlook- en Gmail-flow worden opgevolgd.':gmail?'Gmail is gekoppeld voor opvolging van reacties.':outlook?'Outlook is gekoppeld voor opvolging van reacties.':process.env.RESEND_WEBHOOK_SECRET?'Webhookconfiguratie aanwezig, maar ontvangst is daarmee nog niet als connector geverifieerd.':'Geen inkomende e-mailkoppeling geregistreerd.'},
