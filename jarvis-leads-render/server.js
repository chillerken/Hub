...
app.get('/health', (_req, res) => {
  res.status(200).json({ ok: true, service: 'jarvis-leads-render', version: '2.0.0', authConfigured: Boolean(ACCESS_CODE && SESSION_SECRET && BACKEND_TOKEN) });
});
...
app.get('/api/leads', async (req, res) => {
  try {
    if (!validSession(getCookie(req, 'jarvis_session'))) return res.status(401).json({ error: 'unauthorized' });
    const upstream = await fetchLeads();
...
<style>
:root{--bg:#030711;--panel:#091421;--line:#173a4d;--cyan:#38eaff;--green:#65e6a1;--red:#ff7584;--muted:#82a0b0}
...
<section id="login" class="panel login"><h2 style="margin-top:0">JARVIS-login</h2><p class="meta">Gebruik je nieuwe JARVIS-toegangscode.</p><div class="toolbar"><input id="password" class="input" type="password" autocomplete="current-password" placeholder="Toegangscode"><button id="loginBtn" class="button primary">Login & verbind</button></div><div id="loginMsg" class="meta"></div></section>
